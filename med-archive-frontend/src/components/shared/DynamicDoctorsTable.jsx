...
export default function DynamicDoctorsTable({ title = 'Medecins', useEtablissementScope = false }) {
  ...
  useEffect(() => {
    ...
    async function loadFormData() {
      try {
        ...
        setServices(unwrapList(servicesResponse).rows);
        setSpecialites(unwrapList(specialitesResponse).rows);
        setNewDoctor((doctor) => ({ ...doctor, service_id: service?.id ? String(service.id) : doctor.service_id }));
      } catch (err) {
  ...
  function updateDoctorField(event) {
    ...
    setNewDoctor((doctor) => ({ ...doctor, [name]: value }));
  }
  ...
  async function handleCreateDoctor(event) {
    ...
    try {
      ...
      setNewDoctor({ ...emptyDoctor, service_id: currentService?.id ? String(currentService.id) : '' });
      await loadDoctors();
        ...
        <div className="doctor-modal-overlay" onClick={() => setIsModalOpen(false)}>
          <form className="doctor-modal" onSubmit={handleCreateDoctor} onClick={(event) => event.stopPropagation()}>
            ...
            <div className="doctor-modal-body">
              ...
              <div className="doctor-form-group"><label>Specialite</label><select name="specialite_id" value={newDoctor.specialite_id} onChange={updateDoctorField} required><option value="">Choisir une specialite</option>{specialites.map((specialite) => <option key={specialite.id} value={specialite.id}>{specialite.nom}</option>)}</select></div>
